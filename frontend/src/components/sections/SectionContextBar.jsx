...
const SectionContextBar = ({ currentAcademicYear, isLoading, totalSections = 0 }) => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 text-xs sm:text-sm">
      ...
      <div className="flex items-center space-x-1.5 text-slate-500">
        ...
        <span className="text-slate-800 font-semibold">Sections Management</span>
      </div>
      ...
      <div className="flex items-center space-x-2 bg-blue-50/70 px-2.5 py-0.5 rounded-full border border-blue-100/60">
        ...
        <span className="text-[10.5px] font-semibold text-slate-600 tracking-wider uppercase">
          {isLoading ? "Syncing..." : `${totalSections} sections`}
        </span>
      </div>
    </div>
  );
